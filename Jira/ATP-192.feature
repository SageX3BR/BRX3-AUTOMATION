...
# Header
# -------------------------------------------------------------------------
# - Test code: ATP-192 - Reforma Tributaria - Cenario Completo de Vendas - SOH > SDH > SIH
# - OBS: Este cenário está considerando Cálculo com Diferimento e informado elementos de faturação Frete e Desconto
# - Jira: X3DEV-8547
...
# - Updated date:
# - Status: Done
# - Parametrizações: RTAX 1023 / Cliente BR016
###########################################################################
...
Feature: ATP-192
    ...
    Scenario Outline: 003. Add Lines
        ...
        Examples:
            ...
            | 1   | "BMS119" | "1" | "100.00" | "6102" | "10"      | "000"   | "000001" | "510"    | "510001" |
    ...
    Scenario: 004. CRIAR ENCOMENDA DE VENDA
        ...
        And the user adds the text "0.5" in selected cell and hits tab key
        And the user selects cell with column header: "% or amount" and row number: 3
        And the user adds the text "3.00" in selected cell and hits tab key
    ...
    Scenario: 005. Validação Impostos Encomenda SOH
        ...
        And the value of the selected text field is "0.50"
        Then the user selects the text field with name: "Freight"
        And the value of the selected text field is "3.00"
        Then the user selects the text field with name: "Base cálculo IS"
        And the value of the selected text field is "102.50"
        Then the user selects the text field with name: "Valor IS"
        And the value of the selected text field is "0.51"
        And the user selects the text field with name: "Base cálculo CBS"
        And the value of the selected text field is "102.50"
        And the user selects the text field with name: "Valor CBS Diferido"
        And the value of the selected text field is "0.28"
        And the user selects the text field with name: "Valor CBS"
        And the value of the selected text field is "0.64"
        And the user selects the text field with name: "Base cálculo IBS Mun."
        And the value of the selected text field is ""
        And the user selects the text field with name: "Valor IBS Diferido Mun."
        And the value of the selected text field is ""
        ...
        And the value of the selected text field is "102.50"
        And the user selects the text field with name: "Valor IBS diferido Est."
        And the value of the selected text field is "0.03"
        And the user selects the text field with name: "Valor IBS Est."
        And the value of the selected text field is "0.07"
    ...
    Scenario: 015. Validar Tags no XML
        ...
        And the value of the selected text field contains "<vBCIS>102.50</vBCIS>"
        And the value of the selected text field contains "<vIBSUF>0.07</vIBSUF>"
        And the value of the selected text field contains "<vDif>0.03</vDif>"
        And the value of the selected text field contains "vCBS>0.64</vCBS>"
        And the value of the selected text field contains "<vDif>0.28</vDif>"
        And the value of the selected text field contains "<vBCIBSCBS>102.50</vBCIBSCBS>"
